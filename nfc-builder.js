(function () {
  ...
  function typeDefaults(type) {
    return { headline: type === "reviews" ? copy[language].reviewDefault : type === "menu" ? copy[language].menuDefault : copy[language].websiteDefault, instruction: copy[language].instructionDefault };
  }
  ...
  function setType(type) {
    ...
    destinationLabel.dataset.i18n = type === "reviews" ? "reviewUrl" : type === "menu" ? "menuUrl" : "websiteUrl";
    destinationLabel.textContent = copy[language][destinationLabel.dataset.i18n];
    fields.destinationUrl.placeholder = type === "reviews" ? "https://g.page/r/.../review" : type === "menu" ? "https://auramenu.space/isletmeniz" : "https://example.com";
    if(type==="menu" && fields.cardSize.value==="small") fields.cardSize.value="large";
    updatePreview();
  }
  ...
  function updatePreview() {
    ...
    document.getElementById("previewIcon").textContent = type === "reviews" ? "★" : type === "menu" ? "▦" : "↗";
    document.getElementById("previewType").textContent = type === "reviews" ? copy[language].reviewType : type === "menu" ? copy[language].menuType : copy[language].websiteType;
    ...
    [document.getElementById("cardFront"),document.getElementById("cardBack")].forEach(card=>{card.classList.toggle("size-large",fields.cardSize.value==="large");card.classList.toggle("corners-square",fields.cornerStyle.value==="square");card.dataset.font=fields.fontStyle.value;card.dataset.layout=fields.layoutStyle.value});
    document.getElementById("physicalSize").textContent=fields.cardSize.value==="large"?"120 × 180 mm":"85.6 × 54 mm";
    const fee=fields.deliveryMethod.value==="delivery"?60:fields.deliveryMethod.value==="onsite"?100:0;
    document.getElementById("orderTotal").textContent=new Intl.NumberFormat(language==="ar"?"ar":language==="en"?"en":"tr-TR").format((Number(fields.quantity.value)||1)*700+fee)+" TL";
    setCardColors();
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const payload = {
      ...
      cardSize: fields.cardSize.value, fontStyle: fields.fontStyle.value, layoutStyle: fields.layoutStyle.value,
      cornerStyle: fields.cornerStyle.value, deliveryMethod: fields.deliveryMethod.value,
