    const NAV = [
      ["overview","Overview"],["customers","Customer Accounts","owner"],["nfc","NFC Design Requests"],["auramenu","AuraMenu Requests"],["quicksite","QuickSite Requests"],["pricing","Pricing"],["packages","Packages"],["services","Services"],["portfolio","Portfolio"],
    ...
    const state={view:"overview",clients:[],editing:null,user:null};
    ...
    const visibleNav=()=>NAV.filter(([, ,role])=>!role||state.user?.role===role);
    ...
    function applyReadOnlyUi(){if(canWrite())return;document.querySelectorAll("#content button").forEach(button=>{if(!button.matches("[data-nfc-details],[data-menu-details]")){button.disabled=true;button.title="Read-only account"}})}
    ...
    async function openView(view){if(!visibleNav().some(([id])=>id===view))view="overview";state.view=view;location.hash=view==="overview"?"":view;buildNav();const meta=RESOURCE[view];$("pageTitle").textContent=meta?.title||({overview:"Overview",customers:"Customer Accounts",nfc:"NFC Design Requests",auramenu:"AuraMenu Requests",quicksite:"QuickSite Requests",pricing:"Pricing",analytics:"Analytics",team:"Team & Access",audit:"Security Log"}[view]||view);$("pageSubtitle").textContent=meta?.subtitle||({overview:"Your business at a glance.",customers:"Create secure customer logins and assign purchased products.",nfc:"Review colors and card details, confirm payment, then approve production.",auramenu:"Verify payment, review the requested menu and approve publishing.",quicksite:"Review customer websites, confirm payment and approve publishing.",pricing:"Control all public starting prices from one place.",analytics:"Privacy-friendly aggregate website traffic for the last 30 days.",team:"Individual accounts, roles and revocable access.",audit:"Recent administrator sign-ins and data changes."}[view]||"");$("content").innerHTML='<section class="panel"><p>Loading…</p></section>';
      try{if(view==="overview")await renderOverview();else if(view==="customers")await renderCustomers();else if(view==="nfc")await renderNfc();else if(view==="auramenu")await renderAuraMenu();else if(view==="quicksite")await renderQuickSite();else if(view==="pricing")await renderPricing();else if(view==="analytics")await renderAnalytics();else if(view==="team")await renderTeam();else if(view==="audit")await renderAudit();else await renderResource(view);applyReadOnlyUi()}catch(error){if(error.status===401)return checkSession();$("content").innerHTML=`<section class="panel"><div class="notice error">${esc(error.message)}</div></section>`}}
    ...
    async function renderPricing(){const data=await api("/api/admin/settings");const map=Object.fromEntries((data.settings||[]).map(item=>[item.key,item.value]));$("content").innerHTML=`<section class="panel"><div class="panel-head"><div><h2>Public starting prices</h2><p>These values are used by the live NFC, QR Menu and website pricing areas.</p></div><span class="status">Live settings</span></div><div class="pricing-form">${PRICE_FIELDS.map(([key,label,help])=>`<label>${esc(label)} (TL)<small>${esc(help)}</small><input type="number" min="0" step="1" data-price-key="${key}" value="${esc(map[key]||0)}" ${canWrite()?"":"disabled"}></label>`).join("")}<div class="wide-actions">${canWrite()?'<button id="savePrices" class="btn btn-dark">Save all prices</button>':'<span class="pill">Read-only access</span>'}<span id="priceNotice" class="notice"></span></div></div></section>`;if(canWrite())$("savePrices").addEventListener("click",savePrices)}
...
async function renderCustomers(){
  const data=await api("/api/admin/customer-accounts");
  const accounts=data.accounts||[];
  const rows=accounts.map(item=>`<tr><td><strong>${esc(item.business_name||item.tenant_name)}</strong><small style="display:block;color:#6b7280">/${esc(item.slug||"—")}</small></td><td>${esc(item.email)}<small style="display:block;color:#6b7280">${esc(item.role)}</small></td><td>${(item.products||[]).map(product=>`<span class="pill ok">${esc(product)}</span>`).join(" ")||"—"}</td><td><span class="pill ${item.user_status==="active"?"ok":"warn"}">${esc(item.user_status)}</span></td><td>${item.last_login_at?esc(new Date(item.last_login_at).toLocaleString()):"Never"}</td><td><div class="row-actions"><a class="btn btn-light btn-sm" href="/customer/" target="_blank">Panel ↗</a><button class="btn btn-light btn-sm" data-reset-customer="${item.user_id}">Reset password</button><button class="btn ${item.user_status==="active"?"btn-danger":"btn-dark"} btn-sm" data-toggle-customer="${item.user_id}" data-status="${item.user_status}">${item.user_status==="active"?"Disable":"Enable"}</button></div></td></tr>`).join("");
  $("content").innerHTML=`<section class="panel"><div class="panel-head"><div><h2>Create customer access</h2><p>Assign only the products the customer purchased. Optional request IDs copy approved AuraMenu or QuickSite content into the customer editor.</p></div><span class="status">Tenant isolated</span></div><form id="customerForm" class="pricing-form"><label>Business name<input name="businessName" required maxlength="120"></label><label>Public slug<input name="slug" required pattern="[A-Za-z0-9-]{3,48}" placeholder="restaurant-name"></label><label>Customer email<input name="email" type="email" required></label><label>Initial password<input name="password" type="password" minlength="12" required><small>12+ characters with a letter and number</small></label><label>Role<select name="role"><option value="customer_admin">Customer admin</option><option value="customer_editor">Customer editor</option></select></label><label>AuraMenu request ID (optional)<input name="auraMenuRequestId"></label><label>QuickSite request ID (optional)<input name="quickSiteRequestId"></label><label class="check"><input name="auramenu" type="checkbox"> AuraMenu editor</label><label class="check"><input name="quicksite" type="checkbox"> QuickSite editor</label><div class="wide-actions"><button class="btn btn-dark" type="submit">Create secure account</button><span id="customerNotice" class="notice"></span></div></form></section><section class="panel"><div class="panel-head"><div><h2>Customer accounts</h2><p>${accounts.length} account${accounts.length===1?"":"s"}</p></div></div>${accounts.length?`<div class="table-wrap"><table class="data-table"><thead><tr><th>Business</th><th>Account</th><th>Products</th><th>Status</th><th>Last login</th><th></th></tr></thead><tbody>${rows}</tbody></table></div>`:'<div class="empty">No customer accounts yet.</div>'}</section>`;
  $("customerForm").addEventListener("submit",createCustomerAccount);
  document.querySelectorAll("[data-reset-customer]").forEach(button=>button.addEventListener("click",()=>resetCustomerPassword(button.dataset.resetCustomer)));
  document.querySelectorAll("[data-toggle-customer]").forEach(button=>button.addEventListener("click",()=>toggleCustomer(button.dataset.toggleCustomer,button.dataset.status)));
}

async function createCustomerAccount(event){event.preventDefault();const form=event.currentTarget,notice=$("customerNotice");const data=Object.fromEntries(new FormData(form));data.products=[];if(form.elements.auramenu.checked)data.products.push("auramenu");if(form.elements.quicksite.checked)data.products.push("quicksite");delete data.auramenu;delete data.quicksite;notice.textContent="Creating…";const button=form.querySelector("button");button.disabled=true;try{await api("/api/admin/customer-accounts",{method:"POST",body:JSON.stringify(data)});notice.className="notice success";notice.textContent="Account created.";await renderCustomers()}catch(error){notice.className="notice error";notice.textContent=error.message}finally{button.disabled=false}}
async function resetCustomerPassword(userId){const value=prompt("Enter a new temporary password (12+ characters, letter + number):");if(!value)return;try{await api(`/api/admin/customer-accounts/${userId}/reset-password`,{method:"POST",body:JSON.stringify({password:value})});alert("Password reset. Existing sessions were signed out.")}catch(error){alert(error.message)}}
async function toggleCustomer(userId,current){if(!confirm(`${current==="active"?"Disable":"Enable"} this customer account?`))return;try{await api(`/api/admin/customer-accounts/${userId}`,{method:"PATCH",body:JSON.stringify({status:current==="active"?"disabled":"active"})});await renderCustomers()}catch(error){alert(error.message)}}
...
async function updateQuickSite(id,patch){
  try{await api("/api/admin/quicksite/"+encodeURIComponent(id),{method:"PATCH",body:JSON.stringify(patch)});await renderQuickSite()}
    ...
    async function renderTeam(){const data=await api("/api/admin/users");const items=data.items||[];const rows=items.map(item=>`<tr><td><strong>${esc(item.displayName)}</strong><small style="display:block;color:#6b7280">@${esc(item.username)}</small></td><td><span class="pill ${item.role==="owner"?"ok":""}">${esc(item.role)}</span></td><td><span class="pill ${item.active?"ok":"warn"}">${item.active?"Active":"Disabled"}</span></td><td>${item.lastLoginAt?esc(new Date(item.lastLoginAt+"Z").toLocaleString()):"Never"}</td><td><button class="btn btn-light btn-sm" data-team-edit="${esc(item.id)}">Edit access</button></td></tr>`).join("");$("content").innerHTML=`<section class="panel"><div class="panel-head"><div><h2>Administrator accounts</h2><p>Use one account per person. Managers can edit business data; viewers cannot make changes; only owners manage access and security logs.</p></div><button id="addAdmin" class="btn btn-dark">+ Add account</button></div><div class="table-wrap"><table class="data-table"><thead><tr><th>Person</th><th>Role</th><th>Status</th><th>Last login</th><th></th></tr></thead><tbody>${rows}</tbody></table></div></section>`;$("addAdmin").addEventListener("click",()=>openTeamEditor(null));document.querySelectorAll("[data-team-edit]").forEach(button=>button.addEventListener("click",()=>openTeamEditor(items.find(item=>item.id===button.dataset.teamEdit))))}

    function openTeamEditor(item){state.editing={resource:"team",item};$("modalTitle").textContent=item?"Edit administrator":"Add administrator";$("modalForm").innerHTML=`${item?`<label>Username<input value="${esc(item.username)}" disabled></label>`:'<label>Username<input name="username" minlength="3" maxlength="64" pattern="[A-Za-z0-9][A-Za-z0-9._-]{2,63}" autocomplete="off" required></label>'}<label>Display name<input name="displayName" minlength="2" maxlength="100" value="${esc(item?.displayName||"")}" required></label><label>Role<select name="role"><option value="viewer" ${item?.role==="viewer"?"selected":""}>Viewer · read only</option><option value="manager" ${item?.role==="manager"?"selected":""}>Manager · can edit</option><option value="owner" ${item?.role==="owner"?"selected":""}>Owner · full access</option></select></label>${item?`<label class="check"><input name="active" type="checkbox" ${item.active?"checked":""}> Active account</label>`:""}<label class="span-2">${item?"New password (leave blank to keep current)":"Temporary password"}<input name="password" type="password" minlength="12" maxlength="200" autocomplete="new-password" ${item?"":"required"}></label><div class="modal-actions"><button type="button" id="cancelModal" class="btn btn-light">Cancel</button><button type="submit" class="btn btn-dark">${item?"Save access":"Create account"}</button></div>`;$("modal").classList.remove("hidden");$("modal").setAttribute("aria-hidden","false");$("cancelModal").addEventListener("click",closeModal)}

    async function saveTeamEditor(event,item){const form=new FormData(event.currentTarget);const body={displayName:String(form.get("displayName")||"").trim(),role:String(form.get("role")||"viewer"),password:String(form.get("password")||"")};if(item)body.active=event.currentTarget.elements.active.checked;else body.username=String(form.get("username")||"").trim();if(item&&!body.password)delete body.password;const resetsCurrentSession=Boolean(item&&item.id===state.user?.id&&body.password);const submit=event.currentTarget.querySelector("button[type=submit]");submit.disabled=true;try{await api("/api/admin/users"+(item?"/"+encodeURIComponent(item.id):""),{method:item?"PATCH":"POST",body:JSON.stringify(body)});if(resetsCurrentSession)return location.reload();closeModal();await renderTeam()}catch(error){alert(error.message)}finally{submit.disabled=false}}

    async function renderAudit(){const data=await api("/api/admin/audit-logs");const items=data.items||[];const rows=items.map(item=>`<tr><td>${esc(new Date(item.createdAt+"Z").toLocaleString())}</td><td><strong>@${esc(item.username)}</strong></td><td><span class="pill">${esc(item.action)}</span></td><td>${esc(item.resource)}</td><td>${esc(item.targetId||"—")}</td><td><small>${esc(item.requestId||"—")}</small></td></tr>`).join("");$("content").innerHTML=`<section class="panel"><div class="panel-head"><div><h2>Administrator audit log</h2><p>Successful sign-ins, failed sign-ins and dashboard changes. No visitor IP addresses are stored.</p></div><span class="status">Last 150 events</span></div>${items.length?`<div class="table-wrap"><table class="data-table"><thead><tr><th>Time</th><th>User</th><th>Action</th><th>Resource</th><th>Target</th><th>Request</th></tr></thead><tbody>${rows}</tbody></table></div>`:'<div class="empty">No security events yet.</div>'}</section>`}
